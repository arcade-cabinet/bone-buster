...
describe("COV4 — prop catalogue", () => {
	it("ships exactly 51 unique props (≥10 satisfies acceptance; pinned to catch accidental deletions)", () => {
		// COV4 shipped 30; PE1 added 4 mansion (library); PE2 added 9
		// farm (courtyard); PE3 added 8 electrical (sewer). Future
		// archetype-scenery slices grow this number — every growth
		// updates the expected count to keep the regression contract
		// strict.
		expect(ALL_PROPS.length).toBe(51);
