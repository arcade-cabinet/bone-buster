...
- [ ] ERR1 Asset-load error modal overlay (arcade-cabinet parity) — DO right after VIS-AUTO (surfaces problems the rest hits). [PRD ERR1]
- [ ] ERR2 **troika-three-text dev-worker crash (live drive 2026-05-28): in-game throws 40+ `Worker module function was called but init did not return a callable function` from `troika-three-text.esm` under Vite dep-optimization — floods console + the leftover "HURT ME PLENTY" ghost text mid-screen after start is a troika field that failed to clear/unmount.** Root-cause the Vite worker bundling (exclude troika from optimizeDeps, or pin the non-suspending primitive's worker init) AND fix the skill-select overlay not unmounting on game start. [live-drive finding]
- [ ] VIS4 Weapon hold transform — center-bottom, aiming forward. **CONFIRMED (live drive): pistol renders bottom-RIGHT, angled across the frame pointing left — exactly the reported defect.** [PRD VIS4]
