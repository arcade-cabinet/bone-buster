...
- [ ] VIS-AUTO **DO FIRST — headed-browser visual harness (user-directed 2026-05-28).** The capture-only `snapshotName:null` screenshot specs are the wrong pattern: bundled-Chromium + `--use-angle=gl` still falls to SwiftShader on CI Linux (renders 3D black), so the team gave up `toHaveScreenshot` assertions — which let a visibly-broken in-game scene ship undetected (see VIS1/VIS4 defects below, found only by hand-driving real Chrome). Adopt the **stellar-descent pattern** (`~/src/arcade-cabinet/stellar-descent/playwright.config.ts` + `tests/e2e/{fixtures/game-fixture,utils/player-governor,visual-regression.spec}.ts`): (1) Playwright project `channel: "chrome"` (system Chrome, real ANGLE/GPU driver — NOT bundled Chromium) + keep GPU_ARGS; CI installs system Chrome via `browser-actions/setup-chrome` (or `playwright install chrome`). (2) An in-game governor/auto-walk debug hook so the harness reaches a LIT, settled gameplay frame (not the skill-picker). (3) Restore real `expect(page).toHaveScreenshot(name, {maxDiffPixels, threshold, animations:"disabled"})` baselines for the deterministic in-game poses (canonical phrase), committed as goldens. (4) `pnpm verify` + CI gate on them. This converts manual eyeballing into the automated regression gate every OVERHAUL2 VIS/HUD item is verified against. [user-directed; siblings: stellar-descent]
