...
- [ ] **ARCH2a** Extract `useEnemyTickLoop` from ObjexoomScene useFrame. Acceptance: a hook in `src/scene/hooks/useEnemyTickLoop.ts` that owns the per-frame enemy AI block (FSM tick → movement application → bullet emission → skeleton melee → mesh transform sync → yuka entity mirror). Inputs: refs for enemies/yukaEntities/bullets/enemyMeshes/lastSeen/aggroFired, plus camera, map, collisionCtx, gameRef, settings, playerPos+velocity. Output: void; mutates refs in place. ObjexoomScene's useFrame calls the hook's per-tick callback. Behavior is byte-identical (no logic changes, only relocation). All 184 unit + 5 browser + 5 e2e screenshot tests stay green.
- [ ] **ARCH2b** Extract `useFireResolution` from ObjexoomScene's onFire handler. Acceptance: a hook in `src/scene/hooks/useFireResolution.ts` that owns the `objexoom:fire` event listener — ray cast, barrel hit-priority over enemy hit, per-pellet shotgun spread, recoil signal, shell ejection, body-part emission on kill. Inputs: refs for map/camera/weapon/ammoRef/gameRef/enemies/barrels/enemyMeshes/explodeBarrelRef, plus settings, muzzleFlashUntil ref, muzzleColorRef. ObjexoomScene becomes orchestration only. All 184+ tests stay green.
