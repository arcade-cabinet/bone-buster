...
- [ ] **PA-MOD7** Muzzle light at the actual barrel tip rather than camera-relative. **D11 supersedes original gltfjsx framing** — the wired GLBs (pistol/chaingun/shotgun/melee_machete) lack muzzle/barrel/tip bones; gltfjsx would have generated typed components over generic node names without delivering the outcome. Acceptance: (a) `WeaponModel` in `src/models.ts` gains a `muzzleOffset: [x,y,z]` in weapon-local space for each of the 4 wired weapons, measured against each GLB's bbox after `rotation`+`autoScale` so the offset points at the visible barrel tip on-screen; (b) `WeaponViewmodel` renders an empty `<group>` at that offset and exposes its world-position via a ref-callback registered on a `MuzzleAnchorContext` provider; (c) `ObjexoomScene`'s muzzle light reads that ref's world-position each frame instead of `camera.position`; (d) screenshot smoke confirms the light bloom visibly originates from the barrel on pistol+shotgun+chaingun firing poses; (e) unit test pins `WeaponViewmodel` registers a non-zero muzzle world-position once `useGLTF` resolves.
