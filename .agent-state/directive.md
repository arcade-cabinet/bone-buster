...
- [~] CR-H1scene Scene HALF DONE (steps a/b/c, see docs/specs/97-scene-decomposition.md): extracted `<ScatterFields>` (12-child scatter cluster), `<EntityMeshes>` (the `<EnemyField>` slot — enemy/pickup/barrel mesh maps), and `runSceneTick()` (the 200-line main frame tick, as a pure fn the unchanged useFrame calls — chosen over a `<SceneTickDriver>` child to keep frame-callback order byte-identical + avoid re-homing the shared-ref web). Scene.tsx 1270→1028. New `bonebuster-sceneTick.test.ts` (8 tests) pins the tick contract. tsc 0, 1304 unit + 17 browser + 5 canonical screenshots green. REMAINING — Shell HALF (step-d): promote GameState to a `gameReducer` + extract `useLevelTransition` from `Shell.tsx` (1063), dissolving the `flushSync` buffering (M-2/M-3). **CR-F8 folds in here** — the `onCollectPickup` table-driven reducer IS part of the `gameReducer` promotion.
...
- [ ] CR-F7 Seed pellet spread via a per-shot forked stream, extract `resolvePellet`/`applyMelee*`/`onEnemyKilled` from `resolveFire` (320 LOC), add a deterministic combat test (full-review F7). Pairs with the resolveFire extraction — do alongside the decomposition phase. NOTE: shell-eject + particle jitter Math.random stays (cosmetic, render-only); only PELLET SPREAD (affects which enemy is hit) gets the reproducible stream.
- [ ] CR-F8 Table-drive `onCollectPickup` (Record<action, (prev,ctx)→{next,effects}>) + per-kind reducer tests (full-review F8). FOLD INTO CR-H1scene's gameReducer promotion — not standalone.
...
- [ ] CR-e2e-hooks `tests/e2e/bonebuster.spec.ts` brand+SEED3-flow rot fixed (landing + new-game-flow tests green again); the 7 gameplay-hook tests (killAllEnemies/enemy-damage/collectKey/portal) still fail. ROOT CAUSE FOUND via live diag: in the Vite DEV server under headless Playwright, `troika-worker-utils` (troika-three-text, via drei `<Text>`) throws "worker module init function failed to rehydrate" / "importScripts failed" — the troika text worker can't spin up, which aborts the in-Canvas Scene subtree's effect registration so the `debugKillAll`/etc listeners never mount (verified: `getState()` reports status=playing, 8 enemies, killAllEnemies is a fn, but kills stay 0 after calling it; fails identically at pre-decomposition d24ee51, so NOT a CR-H1* regression). Spec is NOT in the CI gate (CI runs only the screenshot specs, which pass — production text rendering works). Fix options to weigh: (a) gate troika `<Text>` behind a dev-worker-safe fallback, (b) point the e2e at a preview/prod build instead of `pnpm dev`, (c) make the debug-kill listeners register outside the Canvas subtree so a text-worker fault can't block them. Pick per docs after the gameReducer work.
