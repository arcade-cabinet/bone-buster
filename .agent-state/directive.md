...
- [ ] SEED2 Switch `generateMap` + all procedural consumers to `forkStream(phrase, tag)`; `map.seed:number` → `map.seedPhrase:string`.
  - DONE (uncommitted, PROD tsc=0): engine.generateMap(seedPhrase) — archetype = `cyrb128(phrase)[0]%5`, tool cadence = `cyrb128(phrase)[1]%N`, pickUvHidden forks "ENMX-UV"; all 9 scatter modules + enemyMix → `forkStream(map.seedPhrase, "TAG")`; loot/nature/npc numeric consumers → `cyrb128(map.seedPhrase)[n]`; barrels → forkStream "BARL"; buildMap(seedPhrase); refLevel `seedPhrase: \`reflevel-N\`` (archetype stays index%5); archetype.applyArchetypeOverride rewrites phrases by suffixing; Shell/Scene/useGameRef hold seedPhrase (+`mapSeedNum = cyrb128(seedPhrase)[0]` for cosmetic pickers); urlFlags.readSeedPhraseFromUrl. CANONICAL_SEED_PHRASE = "rotten-cursed-ossuary" → corridor.
  - REMAINING: re-bless ~16 determinism TEST files (≈90 tsc errs). Mechanical: `generateMap(N)`→`generateMap(<phrase>)`, map literals `seed:N`→`seedPhrase:"..."`. Per-archetype test phrases (cyrb128[0]%5): corridor=`test-11`, arena=`test-1`, courtyard=`test-3`, sewer=`test-33`, library=`test-0`. Golden-byte pins (prng/scatter/enemyMix tests) need NEW expected values from the phrase streams — re-bless deliberately. Then remove dead mulberry32/RNG_TAGS/seedFrom/taggedSeed/Seed from prng.ts (cosmetic fns + cyrb stay). Then `pnpm verify` green → commit.
