...
- [ ] **ARCH1b** Migrate every `window.dispatchEvent(new CustomEvent("objexoom:*"))` and `window.addEventListener("objexoom:*")` call to use the typed `dispatch` / `addObjexoomListener` helpers from `src/events.ts`. Convert the five Shell↔Scene channels (`fpsUpdate`, `shake`, `playerHit`, `fellToDeath`, `teleport`) from window-event dispatch to direct ref callbacks on `GameRef` / `SceneRef`. Keep particle-fanout (`burst`, `bodyParts`, `shellEject`), input (`fire`, `jump`, `look`, `move`), and debug (`debugKillAll`, `debugCollectPickups`) channels as window events (multi-consumer broadcast is the correct shape). 185+ unit + 5 browser + 5 e2e screenshot tests stay green.
