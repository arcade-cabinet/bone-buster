...
test.describe("OBJEXOOM screenshots (N1)", () => {
	test("01 landing — DOOM menu", async (_, testInfo) => {
		const baseURL =
	...
	test("02 ingame — flashlight ON (full lighting)", async (_, testInfo) => {
		const baseURL =
			typeof testInfo.project.use.baseURL === "string"
				? testInfo.project.use.baseURL
				: "http://localhost:3000";
	...
	test("03 ingame — flashlight OFF (dark mode)", async (_, testInfo) => {
		const baseURL =
			typeof testInfo.project.use.baseURL === "string"
				? testInfo.project.use.baseURL
				: "http://localhost:3000";
	...
	test("04 going-back strobe — H8/J5", async (_, testInfo) => {
		const baseURL =
			typeof testInfo.project.use.baseURL === "string"
				? testInfo.project.use.baseURL
				: "http://localhost:3000";
	...
	test("05 mission complete — full run cleared", async (_, testInfo) => {
		const baseURL =
			typeof testInfo.project.use.baseURL === "string"
				? testInfo.project.use.baseURL
				: "http://localhost:3000";
