...
- URL flags: `?bonebusterDebug` exposes `window.__bonebuster` for e2e + dev hooks; `?bonebusterSeed=<phrase>` pins the map seed PHRASE (the adjective-adjective-noun map identity; legacy numeric values accepted as a phrase string); `?bonebusterArchetype=<name>` forces the phrase to hash to a chosen archetype (`corridor` / `arena` / `courtyard` / `sewer` / `library`) via `applyArchetypeOverride` suffix-rewriting. Legacy `?objexoomDebug` / `?objexoomSeed` / `?objexoomArchetype` aliases still accepted (R8b shim).
- Screenshot tests (`tests/e2e/screenshots.spec.ts`) use a custom `chromium.launch({ args: [...] })` path with `--use-angle=gl` and CDP `Page.captureScreenshot` (the ANGLE-GL backend is the standing GL choice — keep it). **Correction (2026-05-28): the in-game 3D rendering BLACK in e2e was NOT a SwiftShader shadow-map deadlock (the prior claim) — it was capturing too early, during the wall-clock level-intro/asset-stream window, while `waitForFrames` resolved in milliseconds under headless throttling. Capturing after a real wall-clock wait (or a scene-ready predicate via `page.waitForFunction`) renders the scene LIT.** Wait on real time / a ready signal before an in-game capture; never on a frame count alone. See directive VIS-AUTO.
...
- Deterministic PRNG (SEED1-5, the `~/src/arcade-cabinet` family two-PRNG model — see `docs/specs/96-prng-and-landing.md`): the adjective-adjective-noun seed PHRASE is the map identity. `createMapPrng(phrase)` + per-system `forkStream(phrase, "TAG")` (`src/engine/rng.ts`, seedrandom-backed) drive ALL procedural generation; same phrase → same map. Archetype = `cyrb128(phrase)[0] % 5`; canonical anchor `CANONICAL_SEED_PHRASE = "marrowed-vile-sepulcher"` → corridor (pins the canonical screenshots). A separate EVENT PRNG (buried in Capacitor Preferences, advanced per New Game) drives combat/loot variance + the New Game seed-phrase randomizer. `mulberry32` (`src/engine/prng.ts`) survives ONLY for the numeric-hash-keyed level-name picker + the cosmetic-audio variant RNG — not the map stream.
